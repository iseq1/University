...
class KeypointDescriptor(HoughTransform):
    ...
    def set_keypoint_detection(self, image):
        ...
        print(f'Нашлось {len(keypoints)} ключевых точек')
        return keypoints
    ...
    def get_harris_criteria_filtering(self, file_path, image_to_work=None):
        ...
        print(f"После фильтрации осталось {len(self.keypoints)} ключевых точек")
    ...
    def get_descriptor_from_pyramid(self, filename=None):
        full_data = {}

        pyramid = self.set_image_pyramid(self.gray_image_array)
        for index, img in enumerate(pyramid):
            print(f"Обрабатываю {index+1} уровень пирамиды изображения")
            level_full_data = []
        ...
        self.save_keypoints_data_to_file(full_data, filename)

    def save_keypoints_data_to_file(self, keypoints_data, name):
        if name is not None:
            filename = str(name) + "_keypoints_full_data.json"
        else:
            filename = "keypoints_full_data.json"

        with open(filename, 'w') as f:
            json.dump(keypoints_data, f, ensure_ascii=False, indent=4)

    def load_keypoints_data_from_file(self, filename):
